...
static void parameterize_map_layers(mapnik::Map &m, char * parameter)
{
    // show layers configured to appear
    int n = strlen(parameter);
    for (int i = 0; i < m.layer_count(); i++)
    {
        mapnik::layer& layer = m.getLayer(i);
        if (i<n && parameter[i] == '1')
...
parameterize_function_ptr init_parameterization_function(char * function_name) {
    ...
    } else if (strcmp(function_name, "layers") == 0) {
        return parameterize_map_layers;
    } else {
