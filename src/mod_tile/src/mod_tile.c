...
/* not used any more
// liangdong: compress layers string to a shorter expression of which layers are enabled. An example: 11110111101 
static void encode_layers(request_rec *r, tile_config_rec *tile_config, const char *layerOrg, char *layerEnc, int level)
{
    const char * layerName, * val, * line, * val2, * line2;
    int min_level, max_level;
    int n = 0, isLayerExist = 0;

    line = tile_config->attribution;
    while(*line && (val = ap_getword(r->pool, &line, '@')))
    {
        layerName = ap_getword(r->pool, &val, ',');
        min_level = atoi(ap_getword(r->pool, &val, ','));
        max_level = atoi(val);

        // if level is out of range, do not render
        ...
        // check if layerName is specified inside layerOrg
        line2 = layerOrg, isLayerExist = 0;
        while(*line2 && (val2 = ap_getword(r->pool, &line2, ',')))
        {
            if (strcmp(val2, layerName) == 0)
            {
                isLayerExist = 1;
                break;
            }
    ...
    // close layerEnc string
    layerEnc[n] = 0;
}

// liangdong: new logic for parsing urls
// MUST return 5 if parse success, return 0 if parse failed
static int parse_url(request_rec *r, tile_config_rec *tile_config, char *param, char *extension, int *x, int *y, int *z)
{
    const char * key, * val, * line;
    int paramReady = 0, xReady = 0, yReady = 0, zReady = 0;

    // extension will always be png because renderd currently only supports png
    strcpy(extension, "png");

    // parse parameters seperated by '&'
    line = r->args;
    while(*line && (val = ap_getword(r->pool, &line, '&')))
    {
    ...
    while(*line && (val = ap_getword(r->pool, &line, '&')))
    {
          ...
          {
              encode_layers(r, tile_config, val, param, *z);
              paramReady = 1;
          }
    }

    return (paramReady && xReady && yReady && zReady) ? 5 : 0;
}
*/
    ...
    for (i = 0; i < scfg->configs->nelts; ++i) {
        ...
        if (!strncmp(tile_config->baseuri, r->uri, strlen(tile_config->baseuri))) {
            ...
            if (tile_config->enableOptions) {
                cmd->ver = PROTO_VER;
                // !!! not used !!! liangdong: change the default url processing procedure
                // n = parse_url(r, tile_config, parameters, extension, &(cmd->x), &(cmd->y), &(cmd->z));
                n = sscanf(r->uri+strlen(tile_config->baseuri),"%100[^/]/%d/%d/%d.%255[a-z]/%10s", parameters,&(cmd->z), &(cmd->x), &(cmd->y), extension, option); 
                if (n < 5) {
